...
$find = $database->prepare('SELECT id FROM pruebas WHERE ruta=:ruta');
$find->execute(array(':ruta'=>$relativePath)); $testId=$find->fetchColumn();
if ($testId) {
    $query=$database->prepare('UPDATE pruebas SET titulo=:titulo,curso_codigo=:curso,asignatura=:asig,creador_id=:creador,activa=1,updated_at=CURRENT_TIMESTAMP WHERE id=:id');
    $query->execute(array(':titulo'=>substr($title,0,200),':curso'=>$course,':asig'=>$subject,':creador'=>(int)$_SESSION['admin_id'],':id'=>(int)$testId));
} else {
    $query=$database->prepare('INSERT INTO pruebas(ruta,titulo,curso_codigo,asignatura,creador_id) VALUES(:ruta,:titulo,:curso,:asig,:creador)');
    $query->execute(array(':ruta'=>$relativePath,':titulo'=>substr($title,0,200),':curso'=>$course,':asig'=>$subject,':creador'=>(int)$_SESSION['admin_id'])); $testId=(int)$database->lastInsertId();
}
simceSyncCourses($database);
foreach ($database->query('SELECT id,nombre FROM cursos WHERE activo=1')->fetchAll() as $row) {
    ...
    if(!$check->fetch()) { $assign=$database->prepare('INSERT INTO asignaciones(prueba_id,curso_id) VALUES(:p,:c)'); $assign->execute(array(':p'=>$testId,':c'=>$row['id'])); }
}
