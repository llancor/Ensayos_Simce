...
El instalador usa los archivos locales que están junto al script, instala Apache, PHP y MariaDB con `PDO_MySQL`, conserva las pruebas existentes y migra automáticamente las cuentas y estudiantes de una base SQLite anterior. Las credenciales MySQL se generan aleatoriamente y quedan protegidas en `/var/lib/ensayo-simce/mysql.conf`.
...
Al instalar o actualizar, el gestor mantiene Apache activo pero deshabilita su página predeterminada. Al abrir `http://IP_DEL_SERVIDOR/`, Apache redirige automáticamente a `http://IP_DEL_SERVIDOR/pruebas_educativas/`. El desinstalador restaura el sitio predeterminado únicamente si estaba habilitado antes de instalar Ensayo SIMCE.
...
- Extensión PHP `PDO_MySQL` habilitada y servicio MariaDB activo.
...
- Alumno: `https://SU-DOMINIO/RUTA/portal_estudiante.php`
- Profesor: `https://SU-DOMINIO/RUTA/index.html` → **Docentes**
- Biblioteca docente: `https://SU-DOMINIO/RUTA/menu.html` (requiere sesión)
- Resultados: `https://SU-DOMINIO/RUTA/resultados_dashboard.php` (requiere sesión)
