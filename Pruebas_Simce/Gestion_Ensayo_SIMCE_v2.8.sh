...
APACHE_ENTRYPOINT_CONF="/etc/apache2/sites-available/000-ensayo-simce.conf"
...
source_is_valid(){
  ...
  for required in index.html menu.html pruebas_educativas.html pruebas_educativas.php database.php seguridad.php sesion-cliente.js estudiantes_api.php instalar.php guardar_prueba.php portal_estudiante.php portal_estudiante_api.php resultados_api.php resultados_dashboard.php gestion.php gestion_api.php personalizacion.php personalizacion_api.php marca.js migrar_sqlite_mysql.php api.php; do
...
deploy_files(){
  ...
  info "Copiando la aplicación sin sobrescribir la base, las pruebas ni la personalización existente..."
  rsync -a --delete \
    --exclude='.git/' \
    --exclude='.agents/' \
    --exclude='.codex/' \
...
configure_apache_entrypoint(){
  ...
  config_temp="$(mktemp /tmp/ensayo-simce.XXXXXX)" || die "No se pudo crear el sitio Apache temporal."
  register_temp_dir "$config_temp"
  cat > "$config_temp" <<EOF
# Gestionado por Gestion_Ensayo_SIMCE_v2.8.sh (gestor $MANAGER_VERSION)
<VirtualHost *:80>
    ServerName ensayo-simce.local
...
$route_directives
    ...
    ErrorLog \${APACHE_LOG_DIR}/ensayo-simce-http-error.log
    CustomLog \${APACHE_LOG_DIR}/ensayo-simce-http-access.log combined
</VirtualHost>
EOF
  ...
  a2ensite 000-ensayo-simce >/dev/null || die "No se pudo habilitar la entrada web de Ensayo SIMCE."

  if ! apache2ctl configtest; then
    a2dissite 000-ensayo-simce >/dev/null 2>&1 || true
  ...
  if [[ "$entry_path" == "/" ]]; then
    ok "La IP del servidor abre directamente Ensayo SIMCE."
  else
...
status(){
  ...
  [[ -e /etc/apache2/sites-enabled/000-ensayo-simce.conf ]] && entrypoint_state="aplicación" || entrypoint_state="sitio predeterminado/no configurado"
  if [[ "$admins" == "0" ]]; then
...
diagnose(){
  ...
  check_http_code "Biblioteca sin sesión" "401" "http://127.0.0.1${URL_PATH}/api.php" || failures=$((failures + 1))
...
clave_install() {
    ...
    if [[ ! -f "$KEY_FILE" ]]; then
        warn "No se encontró el archivo setup.key."
        info "Ruta esperada: $KEY_FILE"
        info "La clave aún no se ha generado o la aplicación no está instalada."
        return 1
    fi

    if [[ ! -r "$KEY_FILE" ]]; then
        error "El archivo setup.key existe, pero no se puede leer."
        info "Ruta: $KEY_FILE"
        info "Revise los permisos del archivo."
        return 1
    fi

    echo
    ok "Setup key encontrada:"
    echo
    cat "$KEY_FILE"
    echo
...
menu(){
  ...
  while true; do
	...
	printf '%b15)%b %bClave Instalacion Ensayo SIMCE\n' "$YELLOW" "$NC" "$YELLOW"
	printf '%b0)%b  Salir\n\n' "$YELLOW" "$NC"
