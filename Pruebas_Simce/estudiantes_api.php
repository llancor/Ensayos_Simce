...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($action === 'summary') {
        $total = (int) $database->query('SELECT COUNT(*) FROM alumnos WHERE activo = 1')->fetchColumn();
        $basic = (int) $database->query("SELECT COUNT(*) FROM alumnos WHERE activo = 1 AND nivel = 'basica'")->fetchColumn();
        $middle = (int) $database->query("SELECT COUNT(*) FROM alumnos WHERE activo = 1 AND nivel = 'media'")->fetchColumn();
        $courseRows = $database->query('SELECT curso, idgrado, nivel, COUNT(*) AS total FROM alumnos WHERE activo = 1 GROUP BY curso, idgrado, nivel ORDER BY idgrado, curso')->fetchAll();
    ...
    foreach ($statement->fetchAll() as $row) {
        $students[] = array(
            ...
            'rut' => simceMaskedRut((string) $row['rut']),
...
if ($postAction === 'clear') {
    ...
    try {
        ...
        $currentTotal = (int) $database->query('SELECT COUNT(*) FROM alumnos WHERE activo = 1')->fetchColumn();
        ...
        $delete = $database->prepare('DELETE FROM alumnos');
        $delete->execute();
        $deleted = $delete->rowCount();

        $audit = $database->prepare('INSERT INTO auditoria_administracion (accion, detalle, administrador_id) VALUES (:accion, :detalle, :administrador_id)');
        $audit->execute(array(
...
$select = $database->prepare('SELECT id FROM alumnos WHERE rut_normalizado = :rut');
$insert = $database->prepare('INSERT INTO alumnos (rut, rut_normalizado, nombre, curso, idgrado, nivel) VALUES (:rut, :rut_normalizado, :nombre, :curso, :idgrado, :nivel)');
$update = $database->prepare('UPDATE alumnos SET rut = :rut, nombre = :nombre, curso = :curso, idgrado = :idgrado, nivel = :nivel, activo = 1, updated_at = CURRENT_TIMESTAMP WHERE rut_normalizado = :rut_normalizado');
...
try {
    ...
    foreach ($students as $index => $student) {
        ...
        $select->execute(array(':rut' => $validStudent['rut_normalizado']));
    ...
    $log = $database->prepare('INSERT INTO importaciones_alumnos (archivo, total, insertados, actualizados, rechazados, administrador_id) VALUES (:archivo, :total, :insertados, :actualizados, :rechazados, :administrador_id)');
    $log->execute(array(
        ':archivo' => $filename,
