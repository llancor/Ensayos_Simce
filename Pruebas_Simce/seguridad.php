...
function simceRequireStudent(): array
{
    ...
    if ($id<0) {
        ...
        if ($name==='' || simceStudentListsEnabled($database)) simceJson(array('ok'=>false,'error'=>'El acceso libre ya no está habilitado.'),401);
        $s=$database->prepare('SELECT id AS curso_id,nombre AS curso,idgrado FROM cursos WHERE id=:curso AND activo=1');
        $s->execute(array(':curso'=>$courseId)); $course=$s->fetch();
        if(!$course) simceJson(array('ok'=>false,'error'=>'El curso ya no está disponible.'),401);
        $_SESSION['last_activity']=time();
        return array('id'=>$id,'nombre'=>$name,'curso'=>$course['curso'],'idgrado'=>$course['idgrado'],'curso_id'=>$course['curso_id'],'acceso_libre'=>1);
    }
    $s=$database->prepare('SELECT a.id,a.nombre,a.curso,a.idgrado,c.id AS curso_id FROM alumnos a JOIN cursos c ON c.nombre=a.curso WHERE a.id=:id AND c.id=:curso AND a.activo=1 AND c.activo=1');
