        ...
        function safeServerTestUrl(src) {
            try {
                ...
                return parsed.origin === window.location.origin && parsed.pathname.indexOf('/pruebas/') !== -1 && /\.html?$/i.test(parsed.pathname)
                    ? parsed.href
                    : '';
            } catch (error) {
                return '';
        ...
        document.getElementById('btn-generate').addEventListener('click', () => {
            ...
            const attemptsInput = document.getElementById('cfg-intentos');
            if (!attemptsInput.reportValidity() || attemptsInput.value === '') return;
            const defaultPolicy = { feedback: mostrarResp === 'si', results: document.getElementById('cfg-resultados').value === 'si', attempts: Number(attemptsInput.value), finish: document.getElementById('cfg-finalizar').value };
...
<html lang="es">
<head>
<script type="application/json" id="simce-default-policy">${JSON.stringify(defaultPolicy)}<\/script>
<script>${simceInstallPolicy.toString()};simceInstallPolicy(${JSON.stringify(defaultPolicy)},{});<\/script>
  <meta charset="UTF-8"/>
...
<div class="page">
  ...
  <p style="margin-top:30px; font-size:.75rem; color:#94a3b8; text-align:center; border-top:1px solid #f1f5f9; padding-top:20px;">
    ...
    if (w) {
      ...
      w.document.close();
    if(document.documentElement.classList.contains('simce-no-retry')) w.document.querySelectorAll('.btn-retry-report').forEach(function(b){b.remove();});
  ...
  <script>
    window.onload = function() { setTimeout(function(){ window.print(); }, 500); };
        ...
        window.addEventListener('load', () => {
            ...
            const configIds = ['cfg-titulo', 'cfg-asignatura', 'cfg-asignatura-otra', 'cfg-curso', 'cfg-docente', 'cfg-tiempo', 'cfg-mostrar-resp', 'cfg-resultados', 'cfg-intentos', 'cfg-finalizar', 'cfg-shuffle-alts', 'cfg-filename', 'cfg-puntaje', 'cfg-dificultad', 'cfg-instrucciones', 'cfg-modo-puntaje'];
                        ...
                        for (var i = 0; i < courseKeys.length; i++) {
                            ...
                            for (var j = 0; j < subjectKeys.length; j++) {
                                ...
                                tests.forEach(function(test) {
                                    ...
                                    if (safeUrl) {
                                        html += '<button class="btn btn-primary" style="padding: 0.3rem 0.8rem; font-size: 0.85rem; margin:0;" onclick="loadTestFromServer(' + escHtml(JSON.stringify(safeUrl)) + ')">✏️ Editar</button>';
                                    }
                                    html += '</div>';
                                    html += '</div>';
                                });
        ...
        function loadTestFromServer(url) {
        ...
        function loadRequestedTestForEditing() {
            var params = new URLSearchParams(window.location.search);
            var requestedUrl = params.get('editar');
            if (!requestedUrl) return;
            var safeUrl = safeServerTestUrl(requestedUrl);
            if (!safeUrl) {
                alert('No se pudo abrir la prueba para editar porque su dirección no es válida.');
                return;
            }
            window.history.replaceState(null, '', window.location.pathname);
            loadTestFromServer(safeUrl);
        }

        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', loadRequestedTestForEditing);
        } else {
            loadRequestedTestForEditing();
        }
