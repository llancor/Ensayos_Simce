...
function simceInitializeSchema(PDO $db): void {
    ...
    if (!simceColumnExists($db,'resultados','participante_nombre')) $db->exec("ALTER TABLE resultados ADD COLUMN participante_nombre VARCHAR(150) NULL");
    if (!simceColumnExists($db,'resultados','curso_nombre')) $db->exec("ALTER TABLE resultados ADD COLUMN curso_nombre VARCHAR(80) NULL");
    if (!simceColumnExists($db,'resultados','acceso_libre')) $db->exec("ALTER TABLE resultados ADD COLUMN acceso_libre $bool NOT NULL DEFAULT 0");
    ...
    if (!simceColumnExists($db,'cursos','modo_pruebas')) $db->exec("ALTER TABLE cursos ADD COLUMN modo_pruebas VARCHAR(20) NOT NULL DEFAULT 'todas'");
    if (!simceColumnExists($db,'asignaciones','configuracion')) $db->exec("ALTER TABLE asignaciones ADD COLUMN configuracion TEXT NULL");
    if (!simceColumnExists($db,'resultados','submission_token')) {
        $db->exec('ALTER TABLE resultados ADD COLUMN submission_token VARCHAR(64) NULL');
        $db->exec('CREATE UNIQUE INDEX idx_resultados_token ON resultados(submission_token)');
    }
    simceSyncCourses($db);
}

function simceBranding(PDO $db): array {
    $values=array('nombre_plataforma'=>'Pruebas Educativas','nombre_establecimiento'=>'Nombre Colegio o Liceo','subtitulo'=>'Plataforma Evaluativa','saludo'=>'Hola, bienvenido al sistema','seleccion_perfil'=>'Selecciona tu perfil','logo'=>'','favicon'=>'');
...
function simceStudentListsEnabled(PDO $db): bool {
    $statement=$db->prepare("SELECT valor FROM configuracion WHERE clave='acceso_lista_estudiantes'");
    $statement->execute();
    $value=$statement->fetchColumn();
    return $value===false || !in_array(strtolower(trim((string)$value)),array('0','false','no','off'),true);
}
...
function simceSyncStoredTests(PDO $db): void {
    ...
    foreach(new RecursiveIteratorIterator(new RecursiveDirectoryIterator($root,FilesystemIterator::SKIP_DOTS)) as $file){
        ...
        foreach($db->query("SELECT id,nombre FROM cursos WHERE activo=1 AND modo_pruebas='todas'")->fetchAll() as $course){if(!simceCourseCodeMatches((string)$course['nombre'],$code))continue;$q=$db->prepare('SELECT id FROM asignaciones WHERE prueba_id=:p AND curso_id=:c');$q->execute(array(':p'=>$testId,':c'=>$course['id']));if(!$q->fetch()){$a=$db->prepare('INSERT INTO asignaciones(prueba_id,curso_id) VALUES(:p,:c)');$a->execute(array(':p'=>$testId,':c'=>$course['id']));}}
    }
}
