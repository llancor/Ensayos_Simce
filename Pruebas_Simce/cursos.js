(function () {
    ...
    async function request(body) {
        const response=await window.simceAuthenticatedFetch('gestion_api.php'+(body?'':'?action=summary'),body?{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}:{cache:'no-store'});
    ...
    function renderConfigSummary(course){
        ...
        const custom=assignments.find(a=>a.configuracion); let p=custom&&JSON.parse(custom.configuracion);
        const chip=(text,kind='')=>'<span class="config-chip '+kind+'">'+text+'</span>';
    ...
    function render() {
        ...
        $('managedTests').innerHTML=tests.map(t=>{
            ...
            const p=a&&a.configuracion?JSON.parse(a.configuracion):null;
