...
$courseQuery=$database->prepare("SELECT id,nombre FROM cursos WHERE colegio_id=:school AND activo=1 AND modo_pruebas='todas'");$courseQuery->execute(array(':school'=>$schoolId));
foreach ($courseQuery->fetchAll() as $row) {
    if (!simceCourseCodeMatches((string)$row['nombre'],$course)) continue;
    $check=$database->prepare('SELECT id FROM asignaciones WHERE colegio_id=:school AND prueba_id=:p AND curso_id=:c'); $check->execute(array(':school'=>$schoolId,':p'=>$testId,':c'=>$row['id']));
