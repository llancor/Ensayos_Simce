...
function simceIsAdmin(): bool
{
    ...
    $statement = $database->prepare('SELECT usuario,password_hash,rol,colegio_id FROM administradores WHERE id=:id AND activo=1 LIMIT 1');
    ...
    $_SESSION['colegio_id'] = $admin['colegio_id'] === null ? 0 : (int)$admin['colegio_id'];
...
function simceRequireStudent(): array
{
    ...
    if ($id===0 || $courseId<1 || time()-(int)($_SESSION['last_activity']??0)>10800) simceJson(array('ok'=>false,'error'=>'La sesión del estudiante venció.'),401);
    $schoolId=(int)($_SESSION['colegio_id']??0);
    $database=simceDatabase();
    if($id<0){
        $name=trim((string)($_SESSION['alumno_nombre_libre']??''));
        if($schoolId<1||$name===''||simceStudentListsEnabled($database,$schoolId))simceJson(array('ok'=>false,'error'=>'El acceso libre ya no está habilitado.'),401);
        $s=$database->prepare('SELECT id AS curso_id,nombre AS curso,idgrado,colegio_id FROM cursos WHERE id=:curso AND colegio_id=:school AND activo=1');
        $s->execute(array(':curso'=>$courseId,':school'=>$schoolId));$course=$s->fetch();
        if(!$course)simceJson(array('ok'=>false,'error'=>'El curso ya no está disponible.'),401);
        $_SESSION['last_activity']=time();
        return array('id'=>$id,'nombre'=>$name,'curso'=>$course['curso'],'idgrado'=>$course['idgrado'],'colegio_id'=>$course['colegio_id'],'curso_id'=>$course['curso_id'],'acceso_libre'=>1);
    }
    $s=$database->prepare('SELECT a.id,a.nombre,a.curso,a.idgrado,a.colegio_id,c.id AS curso_id FROM alumnos a JOIN cursos c ON c.nombre=a.curso AND c.colegio_id=a.colegio_id WHERE a.id=:id AND c.id=:curso AND a.colegio_id=:school AND a.activo=1 AND c.activo=1');
    $s->execute(array(':id'=>$id,':curso'=>$courseId,':school'=>$schoolId)); $student=$s->fetch();
