...
# ============================================================
# Gestor de instalación v3.0.0 para Evaluaciones Colegios v2.1.0
...
create_administrator(){
  ensure_user_management_ready
  local username password confirmation profile_option profile exists php_result

  printf '\n%bCREAR USUARIO ADMINISTRADOR%b\n\n' "$BLUE" "$NC"
  read -r -p "Nombre de usuario: " username

  if [[ ! "$username" =~ ^[A-Za-z0-9._@-]{3,80}$ ]]; then
    err "El usuario debe tener entre 3 y 80 caracteres y usar sólo letras, números, punto, guion, guion bajo o @."
    return 1
  fi

  exists="$(mariadb --batch --skip-column-names -u"$MYSQL_USER" -p"$SIMCE_DB_PASSWORD" "$MYSQL_DATABASE" \
    -e "SELECT COUNT(*) FROM administradores WHERE usuario='$(printf "%s" "$username" | sed "s/'/''/g")';" 2>/dev/null || printf '0')"
  if [[ "$exists" != "0" ]]; then
    err "Ya existe un administrador con el usuario '$username'."
    return 1
  fi

  printf '\nSeleccione perfil:\n'
  printf '%b1)%b Docente\n' "$YELLOW" "$NC"
  printf '%b2)%b Administrador\n' "$YELLOW" "$NC"
  printf '%b3)%b Superadmin\n' "$YELLOW" "$NC"
  printf '%b0)%b Cancelar\n\n' "$YELLOW" "$NC"
  read -r -p "Seleccione una opción: " profile_option
  case "$profile_option" in
    1) profile="docente" ;;
    2) profile="administrador" ;;
    3) profile="superadmin" ;;
    0) warn "Creación de usuario cancelada."; return 0 ;;
    *) err "Perfil no válido."; return 1 ;;
  esac
  ...
  php_result="$(printf '%s' "$password" | runuser -u "$WEB_USER" -- env SIMCE_DB_DSN="$SIMCE_DB_DSN" SIMCE_DB_USER="$SIMCE_DB_USER" SIMCE_DB_PASSWORD="$SIMCE_DB_PASSWORD" php -r '
    ...
    $profiles = array("docente", "administrador", "superadmin");
    if (!preg_match("/^[A-Za-z0-9._@-]{3,80}$/", $username)) exit(2);
    if (!in_array($profile, $profiles, true)) exit(3);
    if (strlen($password) < 12 || strlen($password) > 4096 ||
        !preg_match("/[A-Z]/", $password) || !preg_match("/[a-z]/", $password) ||
        !preg_match("/[0-9]/", $password)) exit(4);

    $database = simceDatabase();
    $check = $database->prepare("SELECT COUNT(*) FROM administradores WHERE usuario = :usuario");
    $check->execute(array(":usuario" => $username));
    if ((int)$check->fetchColumn() > 0) exit(5);

    $statement = $database->prepare("INSERT INTO administradores (usuario, password_hash, rol, activo) VALUES (:usuario, :password_hash, :rol, 1)");
    ...
    echo "ok";
  ' "$APP_DIR/database.php" "$username" "$profile" 2>/dev/null)"
  unset password confirmation

  if [[ "$php_result" != "ok" ]]; then
    err "No se pudo crear el administrador. La base no fue modificada."
    return 1
  fi

  ok "Usuario '$username' creado correctamente con perfil '$profile'."
  log "Administrador creado mediante el gestor local: usuario=$username perfil=$profile"
}
...
manage_administrators(){
  ...
  while true; do
    ...
    printf '%b2)%b Crear usuario administrador\n' "$YELLOW" "$NC"
...
menu(){
  ...
  while true; do
	...
	printf '%b15)%b Clave Instalacion Evaluaciones Colegios\n\n' "$YELLOW" "$NC"
	printf '%b0)%b  Salir\n\n' "$YELLOW" "$NC"
