...
# ============================================================
# Gestor de instalación v3.0.2 para Evaluaciones Colegios v2.1.0
...
GITHUB_SUBDIR="${SIMCE_GITHUB_SUBDIR:-evaluaciones_colegios}"
APP_DIR="${SIMCE_APP_DIR:-/var/www/html/evaluaciones_colegios}"
PRIVATE_DIR="${SIMCE_PRIVATE_DIR:-/var/lib/evaluaciones-colegios}"
DB_PATH="${SIMCE_DATABASE_PATH:-$PRIVATE_DIR/evaluaciones_colegios.sqlite}"
MYSQL_DATABASE="${SIMCE_MYSQL_DATABASE:-evaluaciones_colegios_v21}"
MYSQL_USER="${SIMCE_MYSQL_USER:-evaluaciones_app}"
MYSQL_CREDENTIALS="$PRIVATE_DIR/mysql.conf"
URL_PATH="${SIMCE_URL_PATH:-/evaluaciones_colegios}"
WEB_USER="${SIMCE_WEB_USER:-www-data}"
WEB_GROUP="${SIMCE_WEB_GROUP:-www-data}"
BACKUP_DIR="${SIMCE_BACKUP_DIR:-/var/backups/evaluaciones-colegios}"
LOG_FILE="${SIMCE_LOG_FILE:-/var/log/evaluaciones-colegios-manager.log}"
APACHE_SECURITY_CONF="/etc/apache2/conf-available/evaluaciones-colegios-security.conf"
APACHE_VHOST_CONF="/etc/apache2/sites-available/evaluaciones-colegios-vhost.conf"
APACHE_ENTRYPOINT_CONF="/etc/apache2/sites-available/000-evaluaciones-colegios.conf"
APACHE_DEFAULT_SITE_MARKER="$PRIVATE_DIR/apache-default-site.was-enabled"
...
cleanup(){
  ...
  for directory in "${TEMP_DIRS[@]}"; do
    if [[ -n "$directory" && "$directory" == /tmp/evaluaciones-colegios.* ]]; then
      if [[ -d "$directory" ]]; then
        rm -rf -- "$directory"
...
source_is_valid(){
  ...
  for required in index.html database.php seguridad.php estudiantes_api.php instalar.php guardar_prueba.php archivo_prueba.php portal_estudiante.php portal_estudiante_api.php resultados_api.php gestion_v21.php gestion_v21_api.php personalizacion_v21.php personalizacion_v21_api.php plataforma.php plataforma_api.php marca.js api_v21.php; do
...
resolve_deploy_source(){
  ...
  case "$mode" in
    ...
    github)
      ...
      clone_root="$(mktemp -d /tmp/evaluaciones-colegios.XXXXXX)" || return 1
      ...
      if ! source_is_valid "$DEPLOY_SOURCE"; then
        err "La subcarpeta '$GITHUB_SUBDIR' no contiene una distribución completa de Evaluaciones Colegios v2.1.0."
        return 1
      fi
...
install_dependencies(){
  ...
  packages=(apache2 php libapache2-mod-php php-mysql mariadb-server mariadb-client git rsync curl ca-certificates unzip openssl)
  missing=()
...
backup_app(){
  ...
  stage="$(mktemp -d /tmp/evaluaciones-colegios.XXXXXX)" || die "No se pudo crear el directorio temporal."
  register_temp_dir "$stage"
  archive="$BACKUP_DIR/evaluaciones-colegios_${stamp}.tar.gz"
  ...
  mariadb-dump --single-transaction -u"$MYSQL_USER" -p"$SIMCE_DB_PASSWORD" "$MYSQL_DATABASE" > "$stage/datos/evaluaciones_colegios.sql" \
    || die "No se pudo respaldar MySQL."
  chmod 600 "$stage/datos/evaluaciones_colegios.sql"
  [[ -f "$APACHE_SECURITY_CONF" ]] && cp -a -- "$APACHE_SECURITY_CONF" "$stage/apache/"
  [[ -f "$APACHE_VHOST_CONF" ]] && cp -a -- "$APACHE_VHOST_CONF" "$stage/apache/"
  [[ -f "$APACHE_ENTRYPOINT_CONF" ]] && cp -a -- "$APACHE_ENTRYPOINT_CONF" "$stage/apache/"
...
preflight_databases(){
  # La plataforma Evaluaciones Colegios usa una base MySQL independiente.
  # No se importa automáticamente ninguna base SQLite de Ensayo SIMCE.
  if [[ -f "$DB_PATH" ]]; then
    database_integrity_ok "$DB_PATH" || die "La base SQLite privada no supera la verificación de integridad: $DB_PATH"
  fi
}

deploy_files(){
  [[ -n "$DEPLOY_SOURCE" ]] || die "No hay una fuente preparada para instalar."
  mkdir -p -- "$APP_DIR" "$APP_DIR/data" "$APP_DIR/pruebas" "$APP_DIR/pruebas_colegios" "$APP_DIR/uploads/marca" || die "No se pudo crear la carpeta de instalación."
...
initialize_database(){
  ...
  # Inicializa exclusivamente el esquema MySQL/MariaDB de Evaluaciones Colegios.
  # No migra simce.sqlite ni reutiliza cuentas de la aplicación anterior.
  env SIMCE_DB_DSN="$SIMCE_DB_DSN" SIMCE_DB_USER="$SIMCE_DB_USER" SIMCE_DB_PASSWORD="$SIMCE_DB_PASSWORD" \
    php -r 'require $argv[1]; simceDatabase();' "$APP_DIR/database.php" \
    || die "PHP no pudo inicializar el esquema MySQL."

  ok "Base MySQL/MariaDB independiente activa: $MYSQL_DATABASE (credenciales en $MYSQL_CREDENTIALS)"
}
...
ensure_setup_key(){
  ...
  mkdir -p -- "$APP_DIR/data" || die "No se pudo crear la carpeta data."
  configure_mysql

  admin_count="$(mariadb --batch --skip-column-names -u"$MYSQL_USER" -p"$SIMCE_DB_PASSWORD" "$MYSQL_DATABASE" -e 'SELECT COUNT(*) FROM administradores;' 2>/dev/null || printf 'ERROR')"

  if [[ ! "$admin_count" =~ ^[0-9]+$ ]]; then
    die "No se pudo comprobar la cantidad de administradores; no se generará una clave insegura."
  fi

  if (( admin_count > 0 )); then
    rm -f -- "$setup_key"
    ok "Ya existen $admin_count administrador(es); por seguridad no se genera setup.key."
    return 0
  fi

  if [[ ! -s "$setup_key" ]]; then
    openssl rand -hex 32 > "$setup_key" || die "No se pudo generar la clave inicial."
    chown root:"$WEB_GROUP" "$setup_key" || die "No se pudo asignar el propietario de setup.key."
    chmod 640 "$setup_key" || die "No se pudo proteger setup.key."
    ok "Se generó la clave inicial para crear el primer administrador."
  else
    chown root:"$WEB_GROUP" "$setup_key" >/dev/null 2>&1 || true
    chmod 640 "$setup_key" >/dev/null 2>&1 || true
    warn "La clave inicial ya existía y fue conservada."
  fi

  [[ -s "$setup_key" ]] || die "setup.key no fue creado correctamente."
  warn "Lea la clave desde el servidor con la opción 15 o: sudo cat $setup_key"
}
...
apply_permissions(){
  ...
  mkdir -p -- "$APP_DIR/pruebas" "$APP_DIR/pruebas_colegios" "$APP_DIR/uploads/marca" "$APP_DIR/data" "$PRIVATE_DIR" || die "No se pudieron preparar las carpetas escribibles."
  ...
  chown -R -h "$WEB_USER":"$WEB_GROUP" "$APP_DIR/pruebas_colegios" || die "No se pudo asignar la carpeta de pruebas por colegio."
  find "$APP_DIR/pruebas_colegios" -xdev -type d -exec chmod 770 {} +
  find "$APP_DIR/pruebas_colegios" -xdev -type f -exec chmod 660 {} +
  ...
  [[ -f "$APACHE_DEFAULT_SITE_MARKER" ]] && chown root:root "$APACHE_DEFAULT_SITE_MARKER" && chmod 600 "$APACHE_DEFAULT_SITE_MARKER"
...
configure_apache_security(){
  ...
  validate_configuration
  local config_temp
  config_temp="$(mktemp /tmp/evaluaciones-colegios.XXXXXX)" || die "No se pudo crear la configuración temporal."
  register_temp_dir "$config_temp"
  info "Configurando Apache para proteger la base y los archivos internos..."
  cat > "$config_temp" <<EOF
# Gestionado por Gestion_Evaluaciones_Colegios_v3.0.sh (software v2.1.0)
<Directory "$APP_DIR">
...
<Directory "$APP_DIR/pruebas_colegios">
    Require all denied
</Directory>
...
EOF
  ...
  a2enconf evaluaciones-colegios-security >/dev/null || die "No se pudo habilitar la configuración de seguridad."
...
configure_apache_entrypoint(){
  require_root
  validate_configuration
  local config_temp entry_path app_path document_root route_directives default_site_was_enabled=false
  app_path="${APP_DIR%/}"
  entry_path="${URL_PATH%/}/"
  document_root="$(dirname -- "$app_path")"
  route_directives="    RedirectMatch 302 \"^/\$\" \"$entry_path\"
    Alias \"$entry_path\" \"$app_path/\""
  ...
  config_temp="$(mktemp /tmp/evaluaciones-colegios.XXXXXX)" || die "No se pudo crear el sitio Apache temporal."
  register_temp_dir "$config_temp"
  cat > "$config_temp" <<EOF
# Gestionado por Gestion_Evaluaciones_Colegios_v3.0.sh (gestor $MANAGER_VERSION)
<VirtualHost *:80>
    ServerName evaluaciones-colegios.local
...
$route_directives
    ...
    ErrorLog \${APACHE_LOG_DIR}/evaluaciones-colegios-http-error.log
    CustomLog \${APACHE_LOG_DIR}/evaluaciones-colegios-http-access.log combined
</VirtualHost>
EOF
  [[ -s "$config_temp" ]] || die "No se pudo escribir el sitio de entrada de Apache."
  install -m 640 -o root -g root "$config_temp" "$APACHE_ENTRYPOINT_CONF" || die "No se pudo instalar el sitio de entrada de Apache."
  a2ensite 000-evaluaciones-colegios >/dev/null || die "No se pudo habilitar la entrada web de la aplicación."

  if ! apache2ctl configtest; then
    a2dissite 000-evaluaciones-colegios >/dev/null 2>&1 || true
    die "Apache rechazó la entrada web; se retiró la configuración nueva."
  fi

  if [[ -e /etc/apache2/sites-enabled/000-default.conf ]]; then
    default_site_was_enabled=true
    printf 'enabled\n' > "$APACHE_DEFAULT_SITE_MARKER" || die "No se pudo registrar el estado del sitio predeterminado de Apache."
    chown root:root "$APACHE_DEFAULT_SITE_MARKER" || die "No se pudo proteger el estado del sitio predeterminado."
    chmod 600 "$APACHE_DEFAULT_SITE_MARKER" || die "No se pudo proteger el estado del sitio predeterminado."
  elif [[ -f "$APACHE_DEFAULT_SITE_MARKER" ]]; then
    default_site_was_enabled=true
    chown root:root "$APACHE_DEFAULT_SITE_MARKER" || die "No se pudo proteger el estado del sitio predeterminado."
    chmod 600 "$APACHE_DEFAULT_SITE_MARKER" || die "No se pudo proteger el estado del sitio predeterminado."
  fi
  ...
  if ! apache2ctl configtest; then
    ...
    die "Apache rechazó el cambio; se restauró el sitio predeterminado."
  fi
  ...
  if ! systemctl reload apache2; then
    ...
    die "No se pudo recargar Apache; se restauró la configuración anterior."
  fi

  if [[ "$entry_path" == "/" ]]; then
    ok "La IP del servidor abre directamente Evaluaciones Colegios."
  else
    ok "La página predeterminada de Apache fue desactivada; la raíz redirige a $entry_path"
  fi
}
...
install_or_update(){
  ...
  # v2.1.0 usa una base MySQL nueva; SQLite anterior se ignora deliberadamente.
  info "Instalación de pruebas v2.1.0: no se migrará ni respaldará la base anterior."
  deploy_files
  initialize_database
  ensure_setup_key
  apply_permissions
...
status(){
  require_root
  validate_configuration
  local apache_active apache_enabled mariadb_active php_version pdo_state db_state admins students setup_state ip app_state vhost_state entrypoint_state
  configure_mysql
  load_proxy_config
  ...
  [[ -e /etc/apache2/sites-enabled/000-evaluaciones-colegios.conf ]] && entrypoint_state="aplicación" || entrypoint_state="sitio predeterminado/no configurado"
  if [[ "$admins" == "0" ]]; then
    if [[ -s "$APP_DIR/data/setup.key" ]]; then setup_state="pendiente"; else setup_state="falta clave"; fi
  elif [[ "$admins" =~ ^[1-9][0-9]*$ ]]; then
    setup_state="cerrado"
  fi
  printf '%bEstado de Evaluaciones Colegios%b\n\n' "$BLUE" "$NC"
  printf '  Gestor:             %s\n' "$MANAGER_VERSION"
  printf '  Software:           %s\n' "$SOFTWARE_VERSION"
  printf '  Aplicación:         %s\n' "$app_state"
...
diagnose(){
  ...
  if [[ "$URL_PATH" == "/" ]]; then
    check_http_code "Entrada por IP a la aplicación" "200,301,302" "http://127.0.0.1/" || failures=$((failures + 1))
  else
    check_http_code "Redirección de la IP a la aplicación" "302" "http://127.0.0.1/" || failures=$((failures + 1))
  fi
  check_http_code "API de pruebas protegida sin sesión" "401" "http://127.0.0.1${URL_PATH}/api_v21.php" || failures=$((failures + 1))
...
list_administrators(){
  ...
  if [[ "$admin_count" == "0" ]]; then
    warn "No existen administradores. Use la opción Crear cuenta superadministrador o instalar.php."
    return 0
  fi
  ...
  printf '\n'
  info "Perfiles válidos: docente, colegio_admin y plataforma_superadmin."
}

create_superadministrator(){
  ensure_user_management_ready
  local username password confirmation superadmin_count create_another php_result

  superadmin_count="$(mariadb --batch --skip-column-names -u"$MYSQL_USER" -p"$SIMCE_DB_PASSWORD" "$MYSQL_DATABASE" \
    -e "SELECT COUNT(*) FROM administradores WHERE colegio_id IS NULL AND rol='plataforma_superadmin';" 2>/dev/null || printf 'ERROR')"
  [[ "$superadmin_count" =~ ^[0-9]+$ ]] || { err "No se pudo consultar las cuentas superadministradoras."; return 1; }

  printf '\n%bCREAR CUENTA SUPERADMINISTRADOR DE PLATAFORMA%b\n\n' "$BLUE" "$NC"
  if (( superadmin_count > 0 )); then
    warn "Ya existen $superadmin_count cuenta(s) superadministradora(s) de plataforma."
    read -r -p "¿Desea crear otra? [s/N]: " create_another
    [[ "$create_another" =~ ^[Ss]$ ]] || { warn "Creación cancelada."; return 0; }
  fi

  read -r -p "Nombre de usuario: " username

  if [[ ! "$username" =~ ^[A-Za-z0-9._-]{3,50}$ ]]; then
    err "El usuario debe tener entre 3 y 50 caracteres y usar sólo letras, números, punto, guion o guion bajo."
    return 1
  fi

  printf '\nDebe contener al menos 12 caracteres, mayúscula, minúscula y número.\n'
  read -r -s -p "Contraseña: " password || return 1
  printf '\n'
  read -r -s -p "Confirmar contraseña: " confirmation || { unset password; return 1; }
  printf '\n'
  ...
  php_result="$(printf '%s' "$password" | runuser -u "$WEB_USER" -- env SIMCE_DB_DSN="$SIMCE_DB_DSN" SIMCE_DB_USER="$SIMCE_DB_USER" SIMCE_DB_PASSWORD="$SIMCE_DB_PASSWORD" php -r '
    ...
    if (!preg_match("/^[A-Za-z0-9._-]{3,50}$/", $username)) exit(2);
    if (strlen($password) < 12 || strlen($password) > 4096 ||
        !preg_match("/[A-Z]/", $password) || !preg_match("/[a-z]/", $password) ||
        !preg_match("/[0-9]/", $password)) exit(3);

    $database = simceDatabase();
    $check = $database->prepare("SELECT COUNT(*) FROM administradores WHERE colegio_id IS NULL AND usuario = :usuario");
    $check->execute(array(":usuario" => $username));
    if ((int)$check->fetchColumn() > 0) exit(4);

    $database->beginTransaction();
    try {
        $statement = $database->prepare("INSERT INTO administradores (colegio_id, usuario, password_hash, rol, activo) VALUES (NULL, :usuario, :password_hash, :rol, 1)");
        $statement->execute(array(
          ":usuario" => $username,
          ":password_hash" => password_hash($password, PASSWORD_DEFAULT),
          ":rol" => "plataforma_superadmin"
        ));
        $audit = $database->prepare("INSERT INTO auditoria_administracion (colegio_id, accion, detalle, administrador_id) VALUES (NULL, :accion, :detalle, NULL)");
        $audit->execute(array(
          ":accion" => "crear_superadmin_cli",
          ":detalle" => "Cuenta superadministradora creada mediante el gestor local: " . $username
        ));
        $database->commit();
        echo "ok";
  ...
  ' "$APP_DIR/database.php" "$username" 2>/dev/null)"
  unset password confirmation

  if [[ "$php_result" != "ok" ]]; then
    err "No se pudo crear la cuenta. Compruebe que el usuario no exista y revise la conexión con la base."
    return 1
  fi

  rm -f -- "$APP_DIR/data/setup.key"
  ok "Cuenta '$username' creada con el perfil plataforma_superadmin."
  info "Para ingresar, deje vacío el código de colegio en la pantalla de acceso."
  log "Superadministrador de plataforma creado mediante el gestor local: usuario=$username"
}
...
manage_administrators(){
  ...
  while true; do
    ...
    printf '%b2)%b Crear cuenta superadministrador de plataforma\n' "$YELLOW" "$NC"
    printf '%b3)%b Restablecer contraseña\n' "$YELLOW" "$NC"
    printf '%b0)%b Volver al menú principal\n\n' "$YELLOW" "$NC"
    read -r -p "Seleccione una opción: " user_option
    case "$user_option" in
      1) list_administrators ;;
      2) create_superadministrator ;;
      3) reset_administrator_password ;;
      0) return 0 ;;
...
create_vhost(){
  ...
  config_temp="$(mktemp /tmp/evaluaciones-colegios.XXXXXX)" || die "No se pudo crear el VirtualHost temporal."
  register_temp_dir "$config_temp"
  cat > "$config_temp" <<EOF
# Gestionado por Gestion_Evaluaciones_Colegios_v3.0.sh (software v2.1.0)
<VirtualHost *:$port>
...
$remote_ip_lines
    ...
    ErrorLog \${APACHE_LOG_DIR}/evaluaciones-colegios-error.log
    CustomLog \${APACHE_LOG_DIR}/evaluaciones-colegios-access.log combined
</VirtualHost>
EOF
  ...
  a2ensite evaluaciones-colegios-vhost >/dev/null || die "No se pudo habilitar el VirtualHost."
...
uninstall_app(){
  require_root
  validate_configuration
  local restore_default_site=false
  ...
  backup_app
  [[ -f "$APACHE_DEFAULT_SITE_MARKER" ]] && restore_default_site=true
  rm -rf -- "$APP_DIR"
  a2dissite 000-evaluaciones-colegios >/dev/null 2>&1 || true
  a2dissite evaluaciones-colegios-vhost >/dev/null 2>&1 || true
  a2disconf evaluaciones-colegios-security >/dev/null 2>&1 || true
  rm -f -- "$APACHE_ENTRYPOINT_CONF" "$APACHE_VHOST_CONF" "$APACHE_SECURITY_CONF"
  if [[ "$restore_default_site" == true && -f /etc/apache2/sites-available/000-default.conf ]]; then
    a2ensite 000-default >/dev/null 2>&1 || warn "No se pudo restaurar el sitio predeterminado de Apache."
  fi
  rm -f -- "$APACHE_DEFAULT_SITE_MARKER"
...
Comandos:
  ...
  users                 Gestiona cuentas administrativas
  create-superadmin     Crea una cuenta superadministradora de plataforma
...
clave_install(){
  ...
  local KEY_FILE="$APP_DIR/data/setup.key"
  local admin_count

  [[ -f "$APP_DIR/database.php" ]] || die "La aplicación todavía no está instalada en $APP_DIR."
  configure_mysql

  admin_count="$(mariadb --batch --skip-column-names -u"$MYSQL_USER" -p"$SIMCE_DB_PASSWORD" "$MYSQL_DATABASE" -e 'SELECT COUNT(*) FROM administradores;' 2>/dev/null || printf 'ERROR')"
  [[ "$admin_count" =~ ^[0-9]+$ ]] || die "No se pudo consultar la tabla de administradores."

  if (( admin_count > 0 )); then
    rm -f -- "$KEY_FILE"
    warn "La base '$MYSQL_DATABASE' ya contiene $admin_count administrador(es)."
    warn "Por seguridad no corresponde generar una clave de instalación."
    return 0
  fi
...
menu(){
  ...
  while true; do
    clear 2>/dev/null || true
    printf '%b============================================%b\n' "$CYAN" "$NC"
    printf '%b   GESTOR v%s — EVALUACIONES COLEGIOS v%s%b\n' "$CYAN" "$MANAGER_VERSION" "$SOFTWARE_VERSION" "$NC"
    printf '%b============================================%b\n\n' "$CYAN" "$NC"
    ...
    printf '%b14)%b Desinstalar Evaluaciones Colegios\n' "$YELLOW" "$NC"
    printf '%b15)%b Crear cuenta superadministrador\n' "$YELLOW" "$NC"
    printf '%b16)%b Mostrar clave para instalación web\n\n' "$YELLOW" "$NC"
    printf '%b0)%b  Salir\n\n' "$YELLOW" "$NC"
...
case "${1:-menu}" in
  ...
  create-superadmin) create_superadministrator ;;
