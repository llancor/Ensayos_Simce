...
simceRequireRole(array('plataforma_superadmin', 'colegio_admin', 'docente'));
$database = simceDatabase();
$schoolId=simceTenantId();if(($_SESSION['admin_rol']??'')==='plataforma_superadmin'){$school=simceSchoolByCode($database,(string)($_GET['colegio']??''));$schoolId=$school?(int)$school['id']:0;}if($schoolId<1)simceJson(array('ok'=>false,'error'=>'Selecciona un colegio válido.'),403);
...
simceRequireRole(array('plataforma_superadmin', 'colegio_admin'));
simceRequireCsrf();
$input = simceReadJson();
