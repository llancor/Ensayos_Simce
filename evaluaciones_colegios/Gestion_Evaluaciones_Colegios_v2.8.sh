...
# ============================================================
# Gestión Evaluaciones Colegios v2.8
...
MYSQL_DATABASE="${SIMCE_MYSQL_DATABASE:-evaluaciones_colegios}"
...
configure_apache_security(){
  ...
  cat > "$config_temp" <<EOF
# Gestionado por Gestion_Evaluaciones_Colegios_v2.8.sh
<Directory "$APP_DIR">
...
menu(){
  ...
  while true; do
    clear 2>/dev/null || true
    printf '%b============================================%b\n' "$CYAN" "$NC"
    printf '%b   GESTIÓN EVALUACIONES COLEGIOS — MySQL/Internet%b\n' "$CYAN" "$NC"
    printf '%b============================================%b\n\n' "$CYAN" "$NC"
