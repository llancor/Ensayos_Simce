...
$s=$db->prepare('SELECT p.* FROM pruebas p JOIN asignaciones a ON a.prueba_id=p.id AND a.colegio_id=p.colegio_id WHERE p.id=:p AND a.curso_id=:c AND p.colegio_id=:school AND p.activa=1 AND a.activa=1'); $s->execute(array(':p'=>$id,':c'=>$student['curso_id'],':school'=>$student['colegio_id'])); $test=$s->fetch();
if(!$test){http_response_code(404);exit('Prueba no disponible.');} $path=realpath(__DIR__.DIRECTORY_SEPARATOR.str_replace('/',DIRECTORY_SEPARATOR,(string)$test['ruta'])); $root=realpath(__DIR__);
