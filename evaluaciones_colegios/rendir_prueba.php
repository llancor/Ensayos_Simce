...
$test=simceTestPolicy($db,(int)$student['curso_id'],$id,(int)$student['colegio_id']);
...
if(!$test)simceUnavailable('Esta prueba no esta disponible para tu curso.');
...
$html=(string)file_get_contents(__DIR__.'/'.$test['ruta']);$token=bin2hex(random_bytes(24));$_SESSION['prueba_tokens'][$token]=array('id'=>$id,'inicio'=>time());if(count($_SESSION['prueba_tokens'])>50)array_shift($_SESSION['prueba_tokens']);
$context=array('testId'=>$id,'token'=>$token,'used'=>$used,'name'=>$student['nombre']);$flags=JSON_UNESCAPED_UNICODE|JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT;
...
if(strpos($html,'submit:function(timeout){')!==false){$html=str_replace('submit:function(timeout){','submit:async function(timeout){',$html);$html=str_replace('if(done)return;done=true;','if(done)return;done=true;window.simceBegin();',$html);$adapter=<<<'JS'
...
if(strpos($html,'await window.simceFinish(resultData)')===false){$html=preg_replace('/^([ \t]*resultData\s*=\s*\{[^\r\n]*\};)[ \t]*$/m','$1'."\n    if (!(await window.simceFinish(resultData))) return;",$html,1,$count);if($count!==1)simceUnavailable('Esta prueba necesita actualizarse en el creador antes de rendirse.');}
