...
function simceBibliotecaPath(string $relative, int $schoolId): array
{
    $relative = str_replace('\\', '/', $relative);
    $path = realpath(__DIR__ . '/' . $relative);
    $root = realpath(__DIR__ . '/pruebas_colegios/' . $schoolId);
...
if ($method === 'GET' && $action === 'export') {
    $id = (int) ($_GET['id'] ?? 0);
    $statement = $database->prepare('SELECT titulo,ruta FROM pruebas WHERE id=:id AND colegio_id=:school AND activa=1');
    $statement->execute(array(':id' => $id, ':school' => $schoolId));
    $test = $statement->fetch();
    if (!$test) simceJson(array('ok' => false, 'error' => 'Prueba no encontrada.'), 404);
    [$path, $filename] = simceBibliotecaPath((string) $test['ruta'], $schoolId);
...
if ($action === 'delete') {
    ...
    $statement = $database->prepare('SELECT id,titulo FROM pruebas WHERE id=:id AND colegio_id=:school AND activa=1');
    $statement->execute(array(':id' => $id, ':school' => $schoolId));
    $test = $statement->fetch();
    if (!$test) simceJson(array('ok' => false, 'error' => 'Prueba no encontrada o ya fue borrada.'), 404);
    $database->beginTransaction();
    try {
        $database->prepare('UPDATE pruebas SET activa=0,updated_at=CURRENT_TIMESTAMP WHERE id=:id AND colegio_id=:school')->execute(array(':id' => $id, ':school' => $schoolId));
        $database->prepare('UPDATE asignaciones SET activa=0 WHERE prueba_id=:id AND colegio_id=:school')->execute(array(':id' => $id, ':school' => $schoolId));
        $database->prepare('INSERT INTO auditoria_administracion(colegio_id,accion,detalle,administrador_id) VALUES(:school,:a,:d,:u)')->execute(array(':school' => $schoolId, ':a' => 'borrar_prueba_biblioteca', ':d' => 'Prueba desactivada: ' . $test['titulo'], ':u' => (int) $_SESSION['admin_id']));
        $database->commit();
    } catch (Throwable $e) {
...
if ($action === 'bulk_delete') {
    ...
    $params = array_merge(array($schoolId), $ids);
    $database->beginTransaction();
    try {
        $statement = $database->prepare("SELECT id,titulo FROM pruebas WHERE colegio_id=? AND activa=1 AND id IN ($placeholders)");
        ...
        $activeParams = array_merge(array($schoolId), $activeIds);
        $database->prepare("UPDATE pruebas SET activa=0,updated_at=CURRENT_TIMESTAMP WHERE colegio_id=? AND id IN ($activePlaceholders)")->execute($activeParams);
        $database->prepare("UPDATE asignaciones SET activa=0 WHERE colegio_id=? AND prueba_id IN ($activePlaceholders)")->execute($activeParams);
        $database->prepare('INSERT INTO auditoria_administracion(colegio_id,accion,detalle,administrador_id) VALUES(:school,:a,:d,:u)')->execute(array(':school' => $schoolId, ':a' => 'borrar_pruebas_biblioteca_masivo', ':d' => count($activeIds) . ' pruebas desactivadas desde biblioteca.', ':u' => (int) $_SESSION['admin_id']));
        $database->commit();
    } catch (Throwable $e) {
        if ($database->inTransaction()) $database->rollBack();
...
if ($action === 'import') {
    ...
    $statement = $database->prepare('INSERT INTO pruebas(colegio_id,ruta,titulo,curso_codigo,asignatura,creador_id,activa) VALUES(:school,:ruta,:titulo,:curso,:asig,:creador,1)');
    $statement->execute(array(':school' => $schoolId, ':ruta' => $relativePath, ':titulo' => substr($title, 0, 200), ':curso' => $course, ':asig' => $subject, ':creador' => (int) $_SESSION['admin_id']));
    $testId = (int) $database->lastInsertId();
    simceSyncCourses($database, $schoolId);
    $courseQuery = $database->prepare("SELECT id,nombre FROM cursos WHERE colegio_id=:school AND activo=1 AND modo_pruebas='todas'");
    $courseQuery->execute(array(':school' => $schoolId));
    foreach ($courseQuery->fetchAll() as $row) {
        if (!simceCourseCodeMatches((string) $row['nombre'], $course)) continue;
        $check = $database->prepare('SELECT id FROM asignaciones WHERE colegio_id=:school AND prueba_id=:p AND curso_id=:c');
        $check->execute(array(':school' => $schoolId, ':p' => $testId, ':c' => $row['id']));
        if (!$check->fetch()) $database->prepare('INSERT INTO asignaciones(colegio_id,prueba_id,curso_id) VALUES(:school,:p,:c)')->execute(array(':school' => $schoolId, ':p' => $testId, ':c' => $row['id']));
    }
    $database->prepare('INSERT INTO auditoria_administracion(colegio_id,accion,detalle,administrador_id) VALUES(:school,:a,:d,:u)')->execute(array(':school' => $schoolId, ':a' => 'importar_prueba_biblioteca', ':d' => 'Prueba importada: ' . $title, ':u' => (int) $_SESSION['admin_id']));
